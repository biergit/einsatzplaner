...
function formatHeader(sheet: GoogleAppsScript.Spreadsheet.Sheet, numCols: number): void {
  ...
  range.setFontWeight('bold').setBackground(HEADER_COLOR).setFontColor(HEADER_FONT_COLOR).setHorizontalAlignment('center');
}
...
function buildDokumentationSheet(einstellungen: Einstellungen): GoogleAppsScript.Spreadsheet.Sheet {
  ...
  sheet.setColumnWidth(1, 380); sheet.setColumnWidth(2, 480); sheet.setColumnWidth(3, 480);

  const format = einstellungen.spielformat;
  const wt = einstellungen.spieltage.map(d => TAGESNAMEN[d]).join(', ');
  ...
  const rows: string[][] = [
    ...
    ['', '', ''],
    ['SHEETS IM ÜBERBLICK', '', ''],
    ['Spieler', 'Stammdaten', 'Name, Email, Rang, Aufstellungsänderungen melden, Rolle. Namen und Reihenfolge bestimmen die Saison-Spalten (werden bei "Aufstellungen generieren" synchronisiert).'],
...
const TAGESNAMEN = ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'];
...
function buildSaisonSheet(config: SheetConfig): GoogleAppsScript.Spreadsheet.Sheet {
  ...
  sheet.setColumnWidth(saisonDatumCol(), 100); sheet.setColumnWidth(saisonWochentagCol(), 80);
  sheet.setColumnWidth(saisonGegnerCol(), 200); sheet.setColumnWidth(saisonStartzeitCol(), 80);
  ...
  for (let row = 0; row < dayCount; row++) {
    ...
    sheet.getRange(row + 2, saisonWochentagCol()).setValue(TAGESNAMEN[d.getDay()]);
  }

  const lastRow = dayCount + 1;
  sheet.getRange(2, saisonDatumCol(), lastRow - 1, 1).setNumberFormat('DD.MM.YYYY');
  sheet.getRange(2, saisonStartzeitCol(), lastRow - 1, 1).setNumberFormat('HH:MM');
...
function buildAenderungslogSheet(): GoogleAppsScript.Spreadsheet.Sheet {
  ...
  const numCols = headers.length;
  sheet.getRange(1, 1, 1, numCols).setValues([headers]); formatHeader(sheet, numCols);
  sheet.setColumnWidth(COL_AENDERUNGSLOG.Zeitstempel, 160); sheet.setColumnWidth(COL_AENDERUNGSLOG.Bereich, 150);
  sheet.setColumnWidth(COL_AENDERUNGSLOG.AlterWert, 250); sheet.setColumnWidth(COL_AENDERUNGSLOG.NeuerWert, 250);
...
function buildAllSheets(config: SheetConfig): void {
  ...
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  for (const s of ss.getSheets()) { if (!knownNames.has(s.getName())) ss.deleteSheet(s); }
